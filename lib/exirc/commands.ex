defmodule ExIRC.Commands do
  ...
  defmacro __using__(_) do
    quote do
      ...
      @rpl_created "003"
      @rpl_myinfo "004"
      # Defacto standard for server support
      @rpl_isupport "005"
      # Defacto replacement of "005" in RFC2812
      @rpl_bounce "010"
      @rpl_statsdline "250"
      # @doc """
  ...
  """
  def whois!(user), do: command!([~c"WHOIS ", user])

  @doc """
  Send a WHO request about a channel
  """
  def who!(channel), do: command!([~c"WHO ", channel])

  @doc """
  Send password to server
  """
  def pass!(pwd), do: command!([~c"PASS ", pwd])

  @doc """
  Send nick to server. (Changes or sets your nick)
  """
  def nick!(nick), do: command!([~c"NICK ", nick])

  @doc """
  Send username to server. (Changes or sets your username)
  """
  def user!(user, name) do
    command!([~c"USER ", user, ~c" 0 * :", name])
  end

  @doc """
  Send PONG in response to PING
  """
  def pong1!(nick), do: command!([~c"PONG ", nick])

  @doc """
  Send a targeted PONG in response to PING
  """
  def pong2!(nick, to), do: command!([~c"PONG ", nick, ~c" ", to])

  @doc """
  Send message to channel or user
  """
  def privmsg!(nick, msg), do: command!([~c"PRIVMSG ", nick, ~c" :", msg])
  ...
  """
  def notice!(nick, msg), do: command!([~c"NOTICE ", nick, ~c" :", msg])

  @doc """
  Send join command to server (join a channel)
  """
  def join!(channel), do: command!([~c"JOIN ", channel])
  def join!(channel, key), do: command!([~c"JOIN ", channel, ~c" ", key])

  @doc """
  Send part command to server (leave a channel)
  """
  def part!(channel), do: command!([~c"PART ", channel])

  @doc """
  Send quit command to server (disconnect from server)
  """
  def quit!(msg \\ "Leaving"), do: command!([~c"QUIT :", msg])
  ...
  def invite!(nick, channel) do
    command!([~c"INVITE ", nick, ~c" ", channel])
  end
end
