defmodule ExIRC.UtilsTest do
  ...
  test "Given a local date/time as a tuple, can retrieve get the CTCP formatted time" do
    # Mimics output of :calendar.local_time()
    local_time = {{2013, 12, 6}, {14, 5, 0}}
    assert Utils.ctcp_time(local_time) == "Fri Dec 06 14:05:00 2013"
  end

  test "Can parse a CTCP command" do
    message = ~c":pschoenf NOTICE #testchan :" ++ ~c"#{<<0o001>>}" ++ ~c"ACTION mind explodes!!" ++ ~c"#{<<0o001>>}"
  ...
  test "Parse cloaked user" do
    message = ~c":foo!foo@unaffiliated/foo PRIVMSG #bar Hiya."
  ...
  test "Parse uncloaked (normal) user" do
    message = ~c":foo!foo@80.21.56.43 PRIVMSG #bar Hiya."
  ...
  test "Parse INVITE message" do
    message = ~c":pschoenf INVITE testuser #awesomechan"
  ...
  test "Parse KICK message" do
    message = ~c":pschoenf KICK #testchan lameuser"
  ...
  test "Can parse RPL_ISUPPORT commands" do
    message = ~c":irc.example.org 005 nick NETWORK=Freenode PREFIX=(ov)@+ CHANTYPES=#&"
  ...
  test "Can parse full prefix in messages" do
    assert %ExIRC.Message{
             ...
             user: "jto",
             host: "tolsun.oulu.fi"
           } = Utils.parse(~c":WiZ!jto@tolsun.oulu.fi NICK Kilroy")
  end
  ...
  test "Can parse prefix with only hostname in messages" do
    assert %ExIRC.Message{
             nick: "WiZ",
             host: "tolsun.oulu.fi"
           } = Utils.parse(~c":WiZ!tolsun.oulu.fi NICK Kilroy")
  end

  test "Can parse reduced prefix in messages" do
    assert %ExIRC.Message{
             nick: "Trillian"
           } = Utils.parse(~c":Trillian SQUIT cm22.eng.umd.edu :Server out of control")
  end

  test "Can parse server-only prefix in messages" do
    assert %ExIRC.Message{
             server: "ircd.stealth.net"
           } = Utils.parse(~c":ircd.stealth.net 302 yournick :syrk=+syrk@millennium.stealth.net")
  end
  ...
  test "Can parse FULL STOP in username in prefixes" do
    assert %ExIRC.Message{
           ...
           } = Utils.parse(~c":nick!user.name@irc.example.org PART #channel")
  end
  ...
  test "parse join message" do
    message = ~c":pschoenf JOIN #elixir-lang"
  ...
  test "Parse Slack's inappropriate RPL_TOPIC message as if it were an RPL_NOTOPIC" do
    ...
    message = ~c":irc.tinyspeck.com 332 jadams #elm-playground-news :"
